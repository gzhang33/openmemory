...
OpenMemory supports Codex with simple configuration. Download the installation scripts and run them to automatically configure your Codex setup:

**For Windows:**
```bash
curl -o install-codex.ps1 http://localhost:3000/scripts/install-codex.ps1
.\install-codex.ps1 -ApiBase "http://localhost:8765" -UserId "your-user-id"
```

**For macOS/Linux:**
```bash
curl -o install-codex.py http://localhost:3000/scripts/install-codex.py
python3 install-codex.py --api-base "http://localhost:8765" --user-id "your-user-id"
```
